...
public class TestSpeechToText {
	private static final String USER_NAME = "555e11bb-bac0-496d-aca2-13b99a741a38";
	private static final String PASSWORD = "3zot0sRBUSoz";
	...
	private void setService() {
		...
		//service = new SpeechToText(USER_NAME, PASSWORD);
	...
	private void getModel() {
		...
		List<SpeechModel> speechModels = serviceCall.execute();
		
		// watson에서 제공하는 모든 언어 모델
		for(SpeechModel model : speechModels) {
			System.out.println(model);
		}
		
		// 선택한 언어에 해당하는 모델을 검색
		// en-US_NarrowbandModel (영어)
		ServiceCall<SpeechModel> serviceCall_eng = service.getModel("en-US_NarrowbandModel");
...
//					}
	...
	private void executeService() {
		try {
			...
			//FileInputStream fis = new FileInputStream(new File("D:/obama_speech.flac"));
			FileInputStream fis = 
					new FileInputStream(
