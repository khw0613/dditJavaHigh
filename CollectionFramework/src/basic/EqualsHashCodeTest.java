...
public class EqualsHashCodeTest {
	 ...
	 * equals()메서드와 hashCode()메서드를 호출한다.
	 * 그래서 객체가 같은지 여부를 결정하려면 두 메서드를 재정의 해야한다.
	 * HashSet, HashMap, HashTable에서는 객체가 긑은지 여부는 데이터를 추가할 때 검사한다.
	 ...
	 * 2. 두 객체가 같은면 equals()메서드를 호출했을 때 true를 반환해야 한다.
	 *  즉, 객체 a, b가 같으면  a.euquals(b)와. b.equals(a) 둘다 true이여야 한다.
	 ...
	 * 5. hashCode()는 기본적으로 Heap에 있는 각 객체에 대한 메모이 주소 매핑 정보를 기반으로 한 정수
	 *  값을 반환한다.
	 *  그러므로, 클래스에서 hashCode()메서드를 override하지 않으면 절대로 두 객체가 같은 것으로 간주될수 없다.
	 *  
	 *  - hashCode()메서드에서 사용하는 '해싱 알고리즘'에서 서로다른 객체에 대하여 같은 hashCode값을
	...
	public static void main(String[] args) {
		...
		p1.setName("일지매"); //추가된 후에 데이터가 변경되어도 Set의 데이터는 변경하지 않는다.
