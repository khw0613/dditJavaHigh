 ...
 * => 외부에서new명령을 사용하지 못하고
 * 동일한 객체(인스턴스)룰 반환하는 클래스
 * 
 * - singleton Class 구성하는 방법
 * 1. 자기 자신 class의 참조변수를 멤버변수로 선언한다.
 * (이 변수는 private static으로 지정한다.)
 * 2. 생성자를 private으로 한다.
 * (외부에서 생성자를 접근을 못하게 하기 위해서 즉, 외부에서
 * new 명령을 사용하게 하지 못하게 하기 위해서)
 * 
 * 3.객체(인스턴스)는 내부에서 생성해서 이 생성된 객체를 반환하는 메서드를 만든다.
 * (이 메서드이 이름은 보통 getInstance()로 지정한다
 * 이 메서드는 static으로 지정한다.)
 *
 */
