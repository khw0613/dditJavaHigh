...
public class PagingController implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		int totPageCount = allTableData.size() % itemsForPage == 0 ? allTableData.size() / itemsForPage
				: allTableData.size() / itemsForPage + 1;//3항 연산자
		 ...
		 * 
		 * @Override public Node call(Integer pageIndex) { from = pageIndex *
		 * itemsForPage; to = from + itemsForPage - 1;
		 ...
		 * 
		 * from = pageIndex * itemsForPage; to = from + itemsForPage - 1;
		 * tableView.setItems(getTableViewData(from, to));
		 * 
		 * return tableView; }); // 페이징처리를 위한 팩토리 메서드 설정
		 */
